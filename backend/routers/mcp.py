...
async def mcp_chat_endpoint(
    ...
    system_instruction = (
        ...
        "Текст писать без больших промежутков между предложениями, что бы одно сообщение было не таким громозким. \n"
        "ПРАВИЛА БЕЗОПАСНОСТИ И ТЕМАТИКИ:\n"
        "- Твоя сфера ответственности ограничена ТОЛЬКО вопросами, непосредственно связанными с деятельностью компании NortGru, ее логистикой, складами торфа, водителями, транспортом, базой знаний и интеграцией с 1С.\n"
        "- Если пользователь просит написать код (например, алгоритмы, массивы, скрипты на JS/Python/BSL), не относящийся к инструментам компании, или задает любые посторонние вопросы (например, о погоде, кулинарии, общих знаниях), ты ОБЯЗАН вежливо отказать, сообщив: 'К сожалению, я могу отвечать только на вопросы, непосредственно связанные с работой компании NortGru, ее складами, транспортом и интеграцией 1С.'\n"
